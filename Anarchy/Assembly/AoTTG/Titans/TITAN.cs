...
public partial class TITAN : TitanBase
{
    ...
    private void Hit(string animationName, Vector3 attacker, float hitPauseTime)
    {
        ...
        needFreshCorePosition = true;
        if (BasePV.IsMine && grabbedTarget != null)
        {
    ...
    private void PlaySound(string sndname)
    {
        ...
        if (BasePV.IsMine)
        {
            BasePV.RPC("playsoundRPC", PhotonTargets.Others, sndname);
        }
    }
    ...
    public void update()
    {
        ...
        if (!hasDie)
        {
            if (state == TitanState.Hit)
            {
                ...
                if (baseA[hitAnimation].normalizedTime >= 1f)
                {
                    Idle();
                }
            }
            ...
            if (!nonAI)
            {
                ...
                if ((state == TitanState.Idle || state == TitanState.Chase || state == TitanState.Wander) &&
                    whoHasTauntMe == null && Random.Range(0, 100) < 10)
                {
            ...
            switch (state)
            {
                    ...
                    if (attackAnimation == "throw")
                    {
                        if (!attacked && baseA["attack_" + attackAnimation].normalizedTime >= 0.11f)
                        {
                            ...
                            if (throwRock.GetComponent<EnemyfxIDcontainer>())
                            {
                                if (nonAI)
                                {
                                    throwRock.GetComponent<EnemyfxIDcontainer>().myOwnerViewID = BasePV.viewID;
                                }
                ...
                case TitanState.Turn:
                    ...
                    if (baseA[turnAnimation].normalizedTime >= 1f)
                    {
                        Idle();
                    }
                ...
                case TitanState.Sit:
                    ...
                    if ((getdownTime <= 0f || isAlarm) && baseA.IsPlaying("sit_idle"))
                    {
