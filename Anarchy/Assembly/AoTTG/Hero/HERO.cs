...
public partial class HERO : HeroBase
{
    ...
    public int Shoots
    {
        ...
        {
            _shoots += value;
            if (_shoots > 16 && !BasePV.owner.AbuseInformation.InfiniteBullets)
            {
                BasePV.owner.AbuseInformation.InfiniteBullets = true;
            }
        }
    }
    ...
    public HeroState State
    {
        ...
        private set
        {
            if (state == HeroState.AirDodge || state == HeroState.GroundDodge)
            {
                dashTime = 0f;
            }
    ...
    private void BombUpdate()
    {
        if (InputManager.IsInputDown[InputCode.Attack1] && skillCDDuration <= 0f)
        {
            ...
            if (Physics.Raycast(ray, out var hitInfo, 1000000f, Layers.EnemyGround.value))
            {
    ...
    private void BufferUpdate()
    {
        if (buffTime > 0f)
        {
            ...
            {
                buffTime = 0f;
                if (currentBuff == Buff.SpeedUp && baseA.IsPlaying("run_sasha"))
                {
                    CrossFade("run", 0.1f);
                }
    ...
    private void FixedUpdate()
    {
        if (isCannon || titanForm ||
            IN_GAME_MAIN_CAMERA.isPausing && IN_GAME_MAIN_CAMERA.GameType == GameType.Single)
        {
            return;
        }
        ...
        if (!IsLocal)
        {
            if (BasePV.owner != null)
            {
                if (gasUsageMax == 0f)
                {
                    int? max = BasePV.owner.Properties[PhotonPlayerProperty.statGAS] as int?;
                    if (max.HasValue)
                    {
                        gasUsageMax = max.Value + 5;
                    }
                }
                if (gasUsageMax > 0 && smoke3Dmg.enableEmission)
                {
                    if (gasUsageTrack > gasUsageMax)
                    {
                        BasePV.owner.AbuseInformation.InfiniteGas = true;
                    }
                    gasUsageTrack += (UseGasSpeed * Time.deltaTime);
                }
            }
            return;
        }
        ...
        {
            current = bulletRight.baseT.position - baseT.position;
        }
 ...
 Mathf.Lerp(IN_GAME_MAIN_CAMERA.BaseCamera.fieldOfView, 50f, 0.1f);
    ...
    private void OnDestroy()
    {
        ...
        if (IsLocal)
        {
            ...
            if (bulletRight)
            {
                Destroy(bulletRight.gameObject);
            }
        }
    }
    ...
    private void ShowAimUI()
    {
        ...
        if (Physics.Raycast(ray, out var raycastHit, 1E+07f, Layers.EnemyGround.value))
        {
            ...
            if (Settings.DisableHookArrrows.Value)
            {
                var localPosition = Vectors.up * 10000f;
    ...
    private void Start()
    {
        ...
        if (IN_GAME_MAIN_CAMERA.GameType == GameType.MultiPlayer)
        {
            ...
            else
            {
                ...
                if (!Settings.HideName.Value || !BasePV.IsMine)
                {
    ...
    public void BackToHuman()
    {
        ...
        if (IN_GAME_MAIN_CAMERA.GameType != GameType.Single)
        {
            BasePV.RPC("backToHumanRPC", PhotonTargets.Others);
        }
    }
    ...
    public void Launch(Vector3 des, bool left = true, bool leviMode = false)
    {
        ...
        if (!leviMode)
        {
            if (a.y < 30f)
            {
                launchForce += Vectors.up * (30f - a.y);
            }

            if (des.y >= baseT.position.y)
            {
                launchForce += Vectors.up * (des.y - baseT.position.y) * 10f;
            }
    ...
    public void update()
    {
        ...
        {
            bool rebindedTrigger = InputManager.LegacyGasRebind.Value ?
                InputManager.IsInputRebindHolding((int)InputRebinds.GasBurst)
                : InputManager.IsInputRebind((int)InputRebinds.GasBurst);
            if (rebindedTrigger)
            {
        ...
        switch (state)
        {
                    ...
                    !Gunner)
                {
                    ...
                    if (InputManager.IsInputDown[InputCode.Attack1])
                    {
                        ...
                        else
                        {
                            ...
                            switch (skillID)
                            {
                                ...
                                default:
                                    if (Settings.BodyLeanEnabled.Value && needLean)
                                    {
                                        if (InputManager.IsInput[InputCode.Left])
                                        {
                    ...
                    else if (InputManager.IsInputDown[InputCode.Attack0])
                    {
                        ...
                        else if (LastHook != null)
                        {
                            ...
                            else if (Settings.BodyLeanEnabled.Value)
                            {
                                flag2 = true;
                            }
                            else
            ...
            case HeroState.ChangeBlade:
                if (Gunner)
                {
                    ...
                    if (baseA[reloadAnimation].normalizedTime > 1f)
                    {
                        idle();
                    }
                }
            ...
            case HeroState.GroundDodge:
                ...
                {
                    if (!grounded && baseA["dodge"].normalizedTime > 0.6f)
                    {
                        idle();
                    }
        ...
        ShowFlareCD();
        ShowAimUI();
        if (InputManager.DisableMouseReeling.Value == false)
        {
