...
public partial class HERO
{
    ...
    private void netCrossFade(string aniName, float time, PhotonMessageInfo info)
    {
        ...
        switch (aniName)
        {
            ...
            case "changeBlade_air":
                ...
                if (bladeReloads > 5)
                {
                    if (!BasePV.owner.AbuseInformation.InfiniteBlades)
                    {
                        BasePV.owner.AbuseInformation.InfiniteBlades = true;
                    }
                }
                break;

            case "dash":
                gasUsageTrack += ((gasUsageMax - 5) * 0.04f);
                break;
        }
    }
    ...
    private void netPlayAnimation(string aniName, PhotonMessageInfo info = null)
    {
        if (!Protection.HeroAnimationCheck.Check(aniName))
        {
            if (info != null)
            {
                AntisManager.Response(info.Sender.ID, true, "Invalid HERO anim: " + aniName);
            }
